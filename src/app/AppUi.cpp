...
namespace {
    ...
    // Compile-time switch for visualizing every region produced by AppLayout.
    ...
    //
    // VisualizerSession owns settings validity and state transitions. These
    // helpers only translate keyboard-friendly changes into session commands.
    // =============================================================================
    ...
    // Input polling
    //
    // These helpers translate raylib keyboard state into app actions. They
    // should stay thin so future mouse/UI controls can issue the same commands.
    // =============================================================================
    ...
    // Layout
    //
    // All app-level drawing regions come from this single model. Future mouse
    // hit testing should use the same calculation so input and drawing cannot
    // develop separate assumptions about control positions.
    // =============================================================================
    ...
    // Partition the current screen into stable, non-overlapping UI regions.
    ...
    // Draw the current keyboard reference. This is presentation only; key
    // polling and command dispatch remain in the input helpers above.
...
void AppUi::handleInput(VisualizerSession& session)
{
    handleSettingsInput(session);
    handlePlaybackInput(session);
}

// Draw app-owned UI inside the frame already opened by App. Keeping frame
// lifetime outside this method makes it impossible for UI drawing to create
// mismatched BeginDrawing()/EndDrawing() calls.
void AppUi::draw(
    const VisualizerSession& session,
    const RaylibRenderer& renderer) const
{
    const AppLayout layout =
        calculateAppLayout(GetScreenWidth(), GetScreenHeight());
